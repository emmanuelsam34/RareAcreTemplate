...
export function Packages({ variant = 'light' }: PackagesProps) {
...
function LightPackages() {
  return (
    <div className="space-y-16">
      <div>
        <FadeIn>
          <span className="mb-4 block h-1 w-12 bg-red" />
          <h3 className="font-poppins text-2xl font-semibold capitalize text-ink-dark md:text-3xl">
            Land Verification &amp; Acquisition Advisory
          </h3>
        ...
        <StaggerGroup className="mt-10 grid gap-6 lg:grid-cols-3">
          {verificationPackages.map((pkg) => (
            <StaggerItem key={pkg.name}>
              <article
                className={`flex h-full flex-col overflow-hidden rounded-2xl border bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
                  pkg.featured ? 'border-red shadow-lg' : 'border-black/10 shadow-sm'
                }`}
              >
                ...
                <div className="flex flex-1 flex-col p-7">
                  <p className="font-poppins text-[10px] font-bold uppercase tracking-[0.18em] text-ink/50">
                    Includes
                  </p>
                  <ul className="mt-3 space-y-2.5">
                    {pkg.includes.map((item) => (
                  ...
                  <p className="mt-5 font-poppins text-[10px] font-bold uppercase tracking-[0.18em] text-ink/50">
                    Deliverables
                  </p>
                  <ul className="mt-3 space-y-2.5">
                    {pkg.deliverables.map((item) => (
                      <li key={item} className="flex gap-2 text-sm text-ink">
                        <span className="font-bold text-ink-dark">→</span>
                        {item}
                      </li>
                    ))}
                  </ul>
      ...
      <div>
        ...
        <StaggerGroup className="mt-10 grid gap-6 lg:grid-cols-2">
          {advisoryPackages.map((pkg) => (
            <StaggerItem key={pkg.name}>
              <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="grid flex-1 gap-6 p-8 sm:grid-cols-2">
                  <div>
                    <h5 className="font-poppins text-[10px] font-bold uppercase tracking-[0.18em] text-ink/50">
                      Includes
                    </h5>
                    <ul className="mt-3 space-y-2.5">
                      {pkg.includes.map((item) => (
